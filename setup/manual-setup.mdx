---
title: 'Manual setup'
description: 'When you want to take over from the agent and finish the setup config by hand'
---

Manual setup is what happens when you open the full editor and fill in
the setup config yourself — adding the steps, files, dev server, and
env vars the agent couldn't infer on its own.

You'd reach for manual setup when:

- The agent ran but couldn't finish, and you can finish it.
- Verification failed and you can see exactly what's wrong faster than
  another agent pass.
...
You can switch between the agent and the editor freely — they edit the
same config.

## Where the editor lives

Two ways into the same editor:

1. **During setup**, after the agent pauses on a failed verification:
   the setup dashboard renders the config editor inline, with whatever
   the agent recorded already filled in.
2. **After setup is done**, from the codebase's dashboard: click the
   **⋯** menu → **Edit setup config**. → [Editing setup config](./edit-config.md)

Both routes show the same five sections.

## The mental model

Imagine cloning your repo onto a brand-new Linux machine that has only
Git, Node, common build tools, and not much else. What would you do, in
order, to get the dev server running on a known port?
...
step](./file-steps.md), and a [dev server](./dev-server.md) entry. If any
of those depend on values that aren't in the repo (an API key, a database
...
that you wouldn't put into a regular config file.
→ [Read more](./env-vars.md)
...
The editor has two buttons that matter: **Verify** and **Update** (or
**Save** during initial setup — same thing).

- **Verify** replays your config in a fresh sandbox, end-to-end. Nothing
  is committed to the codebase yet — you can verify as many times as you
  want.
- **Update** / **Save** promotes the verified config as the canonical
  setup for this codebase. It's only enabled once a verify has just
  finished cleanly with no edits since.

The flow is always: edit → Verify → review the result → Update.

![Screenshot of the dashboard's setup config panel in edit mode, with the Verify and Update buttons highlighted at the top of the footer. A note overlay points at the buttons: "Verify replays your config in a fresh sandbox; Update promotes it as the canonical setup once verification passes." A green "Verification passed" banner is visible above the form.](./images/manual-setup-verify-save.png)

If verification fails, you'll see a red banner at the top of the form
naming the step that broke. The step itself in the list will show its
exit code and output inline so you can fix the problem in place and
re-verify.

If you've been waiting more than two minutes and want to bail, a
**Cancel** button appears next to the verifying banner.
